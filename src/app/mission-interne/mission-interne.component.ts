...
export class MissionInterneComponent implements OnInit {
  mission_internes; reco;perimetre; domaines; id; search_mission; nom; equipe; objectifs; observations
  auditeur;debut;fin;confirm;recommendations
  constructor(private mission_interne: mission_data, private recommendation: recommendation_data, private domaine: domaine_data) { }
...
add_mission(utilisateur: NgForm) {
  this.mission_interne.add_mission_interne(utilisateur.value['nom'], utilisateur.value['perimetre'], utilisateur.value['objectifs'], utilisateur.value['debut'],utilisateur.value['fin'],
utilisateur.value['observations'],utilisateur.value['equipe'])
  setTimeout(() => {
    this.get_all()
  }, 1000);
}
...
edit_mission(utilisateur: NgForm) {
  this.id = this.id;
  this.mission_interne.update_mission_interne(this.id, utilisateur.value['nom'], utilisateur.value['perimetre'], utilisateur.value['objectifs'], utilisateur.value['debut'],utilisateur.value['fin'],
  utilisateur.value['reco'],utilisateur.value['observations'],utilisateur.value['equipe']);
  setTimeout(() => {
    this.get_all();
  }, 1000);
...
search_mission_name() {
  if (this.search_mission == "") {
    ...
    this.mission_interne.get_mission_interne_name(this.search_mission).subscribe(
      (response) => {
        this.mission_internes = response;
