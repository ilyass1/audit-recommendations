...
export class UtilisateurComponent implements OnInit {
  ...
  constructor(
    private utilisateurs: utilisateur_data,
    private structures: structure_data
  ) {}
  ...
  add_utilisateur(utilisateur: NgForm) {
    this.utilisateurs.add_utilisateur(
      utilisateur.value["nom"],
      utilisateur.value["prenom"],
      utilisateur.value["type"],
      utilisateur.value["value"].id,
      utilisateur.value["value"].nom
    );
    setTimeout(() => {
      this.getall();
    }, 1000);
  ...
  edit_utilisateur(utilisateur: NgForm) {
    ...
    this.utilisateurs.update_utilisateur(
      this.id,
      utilisateur.value["nom"],
      utilisateur.value["prenom"],
      utilisateur.value["type"],
